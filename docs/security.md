...
- **パス長制限への対処**: 0.9.9 以降は 96 文字を超える場合に `/tmp` などの短いパスへ自動フォールバックします。環境変数 `KIRI_SOCKET_DIR`
  を設定すればフォールバック先を任意ディレクトリに固定でき、共有サーバーでは `/var/run/kiri` のように 0700 ディレクトリを事前に
  作成しておくことで漏洩リスクを抑えられます。
